...
static int epoll_communicate(int clientFd, int serverFd, char *buf, int bufSize, const UserInfo& userInfo){
	...
	//unique_ptr<filter> upFilter(new naive_filter(p));
...
void handle_db_connection(int clientFd, sockaddr_in clientAddr){
	...
	UserInfo userInfo;
	char ipBuf[16];
	userInfo.ip = inet_ntop(AF_INET, &clientAddr.sin_addr, ipBuf, INET_ADDRSTRLEN);
...
printf("a connection from %s, user is %s\n", userInfo.ip.c_str(), userInfo.user.c_str());
	epoll_communicate(clientFd, serverFd, upBuf.get(), buffsize, userInfo);
}
